...
# /////// QUESTION 1 ///////
# 1. What product sold the most (highest quantity)? Is that the same as the product...
# that brought in the most revenue?
...
postrans %>%
  group_by(PROD_NBR) %>%
  summarize(sale_qty = Reduce('+', SLS_QTY)) %>%
  top_n(1)
...
postrans %>% group_by(PROD_NBR) %>%
      summarize(sale_price = Reduce('+', EXT_SLS_AMT)) %>%
      top_n(3) %>%
...
# ANSWER: product number 98650000000000944904150 had 146 returns.
# The product is a wheeled walker.
func_1 <- function(){
...
prodmaster %>%
      inner_join(func_1(), by = c("PROD_NBR" = "PROD_NBR")) %>%
...
postrans %>%
    group_by(PROD_NBR) %>%
    summarize(sale_qty = Reduce('+', EXT_SLS_AMT)) %>%
    top_n(-1)
...
# ANSWER: 145,624 products have no sales record during this 6 month time frame.
# This means only 43,428 different products sold.
...
subq <- function(){
  x <- pharmacymaster %>%
        inner_join(postrans, by = c("PHRMCY_NBR" = "PHRMCY_NBR")) %>%
        inner_join(prodmaster, by = c("PROD_NBR" = "PROD_NBR")) %>%
        inner_join(prodsubcat, by = c("SUB_CAT_CD" = "SUB_CAT_CD")) %>%
